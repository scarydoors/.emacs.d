...
** Why Emacs
Initially, I was using [[https://code.visualstudio.com/][Visual Studio Code]] for programming and I was happy with
it for a very long time but I started to get fed up with needing to switch to my
using my mouse for a lot of tasks and it wasn't as extensible as other
editors out there. Then, I came across [[https://www.vim.org/][Vim]]/[[https://neovim.io/][Neovim]].

So I opted for Neovim, and eventually learned the plethora of key bindings and
became proficient in using them, all was well. However, configuring Neovim was
when things started to get rocky because VimScript was difficult to use and
documentation wasn't very discoverable inside the editor and amidst this I heard
about Emacs.

Looking at Emacs and seeing that it can do many things other than text editing
it appealed to me because I wouldn't have to leave the environment for many
things, and it had a good ecosystem of packages such as =magit=, =org=, and
even =evil= which helped me with transitioning from Vim to Emacs keeping the
same key bindings. Initially when learning Emacs it was daunting because it
didn't look friendly but I followed the tutorial and got up to speed with it and
ended up with a configuration using =evil-mode=.

Emacs is highly extensible similarly to Vim and self-documenting (=C-h= had
everything I needed). Emacs had a built-in Emacs Lisp tutorial which helped me
get familiar with the language and helped me write my configuration. Almost
every aspect of Emacs is customizable which lets you adapt it to your needs
instead of adapting to the text editors limitations (VSCode is the main culprit
here).

I no longer use =evil-mode= because I wanted better compatibility with key
bindings across Emacs and switched to the default key chord system, and with a
little bit of tweaking and some third-party packages editing with Emacs is
almost as fast as editing with Vim key bindings.
  ...
  (use-package lsp-mode
    ...
    (defun my/lsp-mode-setup-completion ()
      (setf (alist-get 'styles (alist-get 'lsp-capf completion-category-defaults))
            '(flex)))
    :hook (((c++-mode elixir-mode c-mode rust-mode js-mode web-mode css-mode typescript-mode) . lsp-deferred)
